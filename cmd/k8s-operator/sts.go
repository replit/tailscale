...
type tailscaleSTSConfig struct {
	...
	// CustomTLSCerts are copied into the proxy state Secret as
	// <hostname>.crt/<hostname>.key entries so the proxy can terminate TLS for
	// externally managed custom hostnames while still using Tailscale-managed
	// certificates for MagicDNS endpoints.
	CustomTLSCerts map[string]*corev1.Secret
...
func (a *tailscaleSTSReconciler) provisionSecrets(ctx context.Context, tailscaleClient tsClient, loginUrl string, stsC *tailscaleSTSConfig, hsvc *corev1.Service, logger *zap.SugaredLogger) ([]string, error) {
	...
	for i := range stsC.Replicas {
		...
		for host, src := range stsC.CustomTLSCerts {
			mak.Set(&secret.Data, host+".crt", append([]byte(nil), src.Data[corev1.TLSCertKey]...))
			mak.Set(&secret.Data, host+".key", append([]byte(nil), src.Data[corev1.TLSPrivateKeyKey]...))
		}
