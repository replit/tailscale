...
type tailscaleSTSConfig struct {
	...
	// CertShareMode, if non-empty, configures tailscaled to use dedicated TLS
	// Secrets for HTTPS endpoints. This is used for externally managed custom
	// certificates that should be read directly from Kubernetes Secrets.
...
func (a *tailscaleSTSReconciler) reconcileSTS(ctx context.Context, logger *zap.SugaredLogger, sts *tailscaleSTSConfig, headlessSvc *corev1.Service, proxySecrets []string) (*appsv1.StatefulSet, error) {
	...
	if sts.CertShareMode != "" {
		container.Env = append(container.Env, corev1.EnvVar{
			Name:  "TS_CERT_SHARE_MODE",
			Value: sts.CertShareMode,
		})
	}
